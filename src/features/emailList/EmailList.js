...
function Tag({ tag }) {
  return (
    <div style={{
      marginRight: '5px', padding: '2px 5px', background: 'gray', borderRadius: '10px', fontSize: '12px', fontWeight: 'normal',
    }}
    >
      {tag}
    </div>
...
function EmailListItem({ email }) {
  ...
  return (
    <EmailListItemWrapper read={email.read} selected={email.selected}>
      <div style={{
        width: '20px', height: '20px', padding: '0 10px 0 14px', display: 'flex', alignItems: 'center',
      }}
      >
        <Checkbox
      ...
      <div style={{
        flex: '1 1 auto', paddingRight: '10px', textAlign: 'left', display: 'flex',
      }}
      >
